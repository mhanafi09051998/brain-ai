...
function getBalances() {
  return new Promise((resolve) => {
    ...
    const options = {
      ...
      port: 443,
      path: `/open/v1/account?${params.toString()}`,
      method: 'GET',
      headers: {
      ...
      res.on('end', () => {
        try {
          ...
          if (result.code === 0 && result.data && result.data.balances) {
            const usefulBalances = result.data.balances.filter(b => parseFloat(b.free) > 0 || parseFloat(b.locked) > 0);
            resolve({ success: true, balances: usefulBalances });
          } else {
            resolve({ success: false, error: result.msg || 'Unknown error' });
          }
        } catch (e) {
