...
def generate_hourly_report(net_profit_usd=18.45, total_24h_usd=142.80, vault_aum_usd=10142.80, trades_count=3):
    ...
    msg = f"""📊 <b>SOLANA LIQUIDITY VAULT — HOURLY YIELD REPORT</b>
📅 <b>Waktu:</b> <code>{date_str} • {time_range}</code>
🤖 <b>Status Algoritma:</b> 🟢 <b>ACTIVE (High-Frequency Liquidity Radar)</b>
🪙 <b>Live SOL Price:</b> <code>${sol_price:,.2f}</code>
━━━━━━━━━━━━━━━━━━━━━━━━━━━

💰 <b>RINGKASAN PENDAPATAN 1 JAM TERAKHIR:</b>
• <b>Net Profit (1 Jam):</b> <code>+${net_profit_usd:,.2f}</code> (+{profit_pct_hour:.2f}%) / <code>+{net_profit_sol:.4f} SOL</code>
• <b>Total Profit Hari Ini (24h):</b> <code>+${total_24h_usd:,.2f}</code> (+{profit_pct_24h:.2f}%) / <code>+{total_24h_sol:.4f} SOL</code>
• <b>Total Nilai Vault (AUM):</b> <code>${vault_aum_usd:,.2f}</code> (~{vault_aum_sol:.2f} SOL)
• <b>Frekuensi Eksekusi:</b> <code>{trades_count} Trades ({trades_count} Win / 0 Loss) — Win Rate 100%</code>

⚡ <b>LOG TRANSAKSI 1 JAM TERAKHIR:</b>
1. <b>SOL/USDC</b> (Scalp Long FVG)
   • Entry: <code>${sol_price - 0.45:.2f}</code> ➔ Exit: <code>${sol_price + 0.20:.2f}</code> | Net: <code>+$8.20</code>
   • Tx: <a href="https://solscan.io">Solscan Verified</a>
2. <b>SOL/USDC</b> (Liquidity Sweep)
   • Entry: <code>${sol_price - 0.30:.2f}</code> ➔ Exit: <code>${sol_price + 0.25:.2f}</code> | Net: <code>+$6.85</code>
   • Tx: <a href="https://solscan.io">Solscan Verified</a>
3. <b>SOL/USDC</b> (Micro Arbitrage)
   • Spread Capture 0.22% | Net: <code>+$3.40</code>
   • Tx: <a href="https://solscan.io">Solscan Verified</a>

🛡️ <b>STATUS MANAJEMEN RISIKO (ACTIVE RISK TELEMETRY):</b>
• <b>Floating Drawdown:</b> <code>0.00%</code> (Semua posisi ditutup ke safe asset)
• <b>Vault Exposure Saat Ini:</b> <code>0.00%</code> (100% modal aman di Vault)
• <b>Max Risk per Trade:</b> Dibatasi ketat <code>1.5%</code> per entry
• <b>Slippage / MEV Protection:</b> Active (Jito Tip Dynamic Route)

━━━━━━━━━━━━━━━━━━━━━━━━━━━
ℹ️ <i>Laporan otomatis diterbitkan setiap 1 jam secara transparan.</i>
🔐 <i>Powered by Claudia Ultra Engine • Gahar Inovasi Teknologi</i>"""
    return msg
