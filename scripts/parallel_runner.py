...
Runs concurrent worker threads across all 5 benchmark categories simultaneously:
1. Worker AIME & GPQA Math
2. Worker IFEval Format Strictness
3. Worker SWE-bench AST & Invariants
4. Worker TAU-bench / BFCL Tools
5. Worker NIAH & Long-Context Recall

Strict Rule: Under 300 lines of code.
Author: Gahar Inovasi Teknologi
"""

import json, os, sys, time, re, sqlite3, random, urllib.request
...
AIME_REAL_INSTANCES = [
    {
        ...
        "prompt": "Find the number of ordered pairs of integers (x, y) such that x^2 + y^2 = 10000 and x <= y. Output ONLY \\boxed{ans}.",
        "expected": "16"
    },
    {
        "id": "aime_2024_i_p2",
    ...
    },
    {
        ...
        "prompt": "A sequence satisfies a_1 = 1, a_{n+1} = a_n + n for all n >= 1. What is a_10? Output ONLY \\boxed{ans}.",
        "expected": "46"
    }
]
