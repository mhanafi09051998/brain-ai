...
# SubtitleCat search and target release names
targets = {
    ...
    "glass-onion-a-knives-out-mystery": "Glass Onion A Knives Out Mystery 2022 1080p"
}
...
for slug, query in targets.items():
    ...
    print(f"\n[SEARCH] Searching subtitlecat for '{query}'...")
    try:
        ...
        if matches:
            detail_url = f"https://www.subtitlecat.com{matches[0]}"
            ...
            # Find Indonesian download link
            id_dl = re.findall(r'href="([^"]+id\.srt|[^"]+indonesian[^"]*)"', det_html, re.IGNORECASE)
            if not id_dl:
                id_dl = re.findall(r'href="(/download/[^"]+id[^"]*)"', det_html, re.IGNORECASE)
            
            if id_dl:
                dl_link = id_dl[0]
                if not dl_link.startswith('http'):
                    dl_link = f"https://www.subtitlecat.com{dl_link}" if dl_link.startswith('/') else f"https://www.subtitlecat.com/{dl_link}"
                
                print(f"  Downloading Indonesian sub from {dl_link}...")
                dl_req = urllib.request.Request(dl_link, headers=headers)
                raw_sub = urllib.request.urlopen(dl_req, timeout=20).read().decode('utf-8', errors='ignore')
                
                clean_vtt, cue_count = sanitize_srt_to_vtt(raw_sub)
                if cue_count > 100:
                    with open(dest, 'wb') as f:
                        f.write(clean_vtt.encode('utf-8'))
                    print(f"  [✓] SUCCESS: Saved {slug}.vtt with {cue_count} synchronized cues ({len(clean_vtt)} bytes)!")
                else:
                    print(f"  [!] Subtitle too short ({cue_count} cues), trying alternative...")
    except Exception as e:
        print(f"  Error searching for {slug}: {e}")
